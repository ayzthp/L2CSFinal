#!/usr/bin/env python3
"""
Left Hand Focus Bisector Tracker with Normal Speed Video Recording
Enhanced version using MediaPipe FaceMesh for exact pupil and nose-tip landmarks
Focused on left hand detection and posture alignment with normal speed video recording
"""
...
class LeftHandFocusBisectorNormalSpeed:
    def __init__(self, save_dir="captures", video_dir="videos"):
        """Initialize the left hand focus bisector tracker with normal speed video recording"""
        print("🚀 Initializing Left Hand Focus Bisector Tracker with Normal Speed Video Recording...")
        ...
        # Video recording parameters for normal speed
        self.is_recording = False
        self.video_writer = None
        self.video_filename = None
        self.recording_start_time = None
        self.recording_frame_count = 0
        self.target_fps = 10.0  # Target FPS for normal playback
        self.frame_times = []  # Track frame timestamps
        
        print("✅ Left Hand Focus Bisector Tracker with Normal Speed Video Recording initialized!")
    ...
    def run(self):
        """Run the left hand focus bisector tracker with normal speed video recording"""
        ...
        print("\n" + "="*60)
        print("🎯 LEFT HAND FOCUS BISECTOR TRACKER WITH NORMAL SPEED VIDEO RECORDING")
        print("="*60)
        print("Controls:")
        print("  'q' - Quit")
        print("  's' - Save frame")
        ...
        print(f"Target recording FPS: {self.target_fps} (for normal playback)")
        print("="*60)
        ...
        try:
            while True:
                ...
                if self.is_recording:
                    ...
                    cv2.putText(frame, f"Target: {self.target_fps} FPS", (frame.shape[1]-150, frame.shape[0]-60), 
                ...
                if self.is_recording and self.video_writer is not None:
                    ...
                    if self.recording_frame_count % 30 == 0:
                        ...
                        if elapsed_time > 0:
                            actual_fps = self.recording_frame_count / elapsed_time
                            # Clamp FPS to reasonable range (5-30 FPS)
                            actual_fps = max(5.0, min(30.0, actual_fps))
                
                cv2.imshow('Left Hand Focus Bisector Tracker with Normal Speed Video Recording', frame)
            ...
            print("🏁 Left Hand Focus Bisector Tracker with Normal Speed Video Recording ended.")

    def start_recording(self, frame_width: int, frame_height: int):
        """Start video recording with normal speed"""
        if not self.is_recording:
            timestamp = int(time.time())
            self.video_filename = f"left_hand_bisector_normal_speed_{timestamp}.mp4"
            video_path = os.path.join(self.video_dir, self.video_filename)
            
            # Initialize video writer with target FPS for normal playback
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            self.video_writer = cv2.VideoWriter(video_path, fourcc, self.target_fps, (frame_width, frame_height))
            self.is_recording = True
            self.recording_start_time = time.time()
            self.recording_frame_count = 0
            self.frame_times = []
            print(f"🎥 Started recording: {self.video_filename}")
            print(f"📹 Recording at {self.target_fps} FPS for normal playback speed")
    ...
    def stop_recording(self):
        ...
        if self.is_recording and self.video_writer is not None:
            ...
            if self.recording_start_time and len(self.frame_times) > 1:
                ...
                print(f"📊 Actual FPS achieved: {actual_fps:.1f}")
            else:
                print(f"🎥 Stopped recording: {self.video_filename}")
...
def main():
    LeftHandFocusBisectorNormalSpeed().run()
